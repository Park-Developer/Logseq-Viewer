...
from LogView_Lib import html_func, calander_func, viewer_config
...
def create_task_Table(doc_address,main_task_list):
    ...
    for idx in range(viewer_config.logView_Date_Range + 3):
        ...
        if idx==0: # Main Task Name Col
            ...
            what_day=calander_func.convert_weekday(idx_date.weekday())
            if what_day=="(Sat)" or what_day=="(Sun)":
                txt_color=viewer_config.weekend_text_color
            else:
                txt_color=viewer_config.basic_text_color
            
            day_info=str(idx_date.month)+"/"+str(idx_date.day)+"<br>"+calander_func.convert_weekday(idx_date.weekday())
    ...
    maintask_name_list=[] # for combo box1
    assignee_list=[] # for combo box2
    ...
    for mt_idx in range(len(main_task_list)):
        ...
        for st_idx in range(subtask_num):
            ...
            # Get assignee class
            assig_class=""
            if sub_task_info["assignee"]!=None:
                for asig in sub_task_info["assignee"]:
                    assig_class = assig_class + "Assignee_" + asig + " "
            else:
                assig_class="Assignee_None"


           
            
            table_row_start='<tr class="Schedule_Table_ROW{0} MAIN {1}_ {2} {3} Priority_{4} {5}">\n'.format(cur_row_idx,
                                                                                            main_taskName,
            ...
            for day_idx in range(viewer_config.logView_Date_Range + 3):
                ...
                # (COL-4) : Assignee 
                    ...
                    if main_task_list[mt_idx]["sub_task_list"][st_idx]["assignee"] != None:
                        assign_lst = main_task_list[mt_idx]["sub_task_list"][st_idx]["assignee"]

                        assignee=""
                        for assig in assign_lst:
                            assignee=assignee+assig+", "

                            # add assignee list
                            assignee_list.append(assig)
    ...
    # Create combo box1 for task filtering
    combo_opt=""
    for mt_name in maintask_name_list:
        combo_opt=combo_opt+'<option value="{0}">{0}</option>\n'.format(mt_name)
    
    # Create combo box2 for assignee filtering
    combo_opt2="" 

    
    for assig_name in assignee_list:
        combo_opt2=combo_opt2+'<option value="{0}">{0}</option>\n'.format(assig_name)

    '''
    [3] CREATE HTML CONFIGURATION
    '''
    ...
    <div>
        ...
        <div class="Button_Part">
            <button class="schedule_pri_All_btn btn_active" onclick="reset_btn_click()">All</button>
        
            <button class="schedule_pri_A_btn" onclick="Pri_A_btn_click()">A</button>
            <button class="schedule_pri_B_btn" onclick="Pri_B_btn_click()">B</button>
            <button class="schedule_pri_C_btn" onclick="Pri_C_btn_click()">C</button>
            <button class="schedule_pri_X_btn" onclick="Pri_X_btn_click()">X</button>
            ...
            <select name="assignee_combo" onchange="assignee_combo_filtering(this)">
                <option value="-- ALL --" selected align="center"> -- ALL -- </option>
...
def create_deadline_todo_Table(doc_address,deadline_tasklist):
    ...
    for idx in range(viewer_config.logView_Date_Range+1):
        ...
        if idx==0: # Main Task Name Col
            ...
            cal_day_info=str(idx_date.month)+"/"+str(idx_date.day)+"<br>"+calander_func.convert_weekday(idx_date.weekday())
    ...
    html_func.create_Html(doc_address,"""
        ...
        <div>
            ...
            <div class="Button_Part">
                <button class="dead_pri_All_btn btn_active" onclick="todo_reset_btn_click()">All</button>
            
                <button class="dead_pri_A_btn" onclick="todo_Pri_A_btn_click()">A</button>
                <button class="dead_pri_B_btn" onclick="todo_Pri_B_btn_click()">B</button>
                <button class="dead_pri_C_btn" onclick="todo_Pri_C_btn_click()">C</button>
                <button class="dead_pri_X_btn" onclick="todo_Pri_X_btn_click()">X</button>
